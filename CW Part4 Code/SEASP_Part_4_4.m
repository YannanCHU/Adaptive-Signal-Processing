...
for i = 1:length(scalingFactors)
    [y_hat, errors, weights] = LMS_tanh([], y, mu, AR_order, 0, scalingFactors(i), 'bias', []);
...
%%
[y_hat, errors, weights] = LMS_tanh([], y, mu, AR_order, 0, scalingFactors(minMSEIdx), 'bias', []);
...
disp("The optimal scaling factor is " + scalingFactors(minMSEIdx));

figure(2);
plot(1:N, y, 1:N, y_hat, 'LineWidth', 2);
title(["Original signal (non-zero mean) and the non-linear one-step ahead prediction with bias", ...
    sprintf("(MSE = %.3f dB, Rp = %.3f dB)", MSE_db, Rp_db) ], 'FontSize', 14);
xlabel("Sample Index", 'FontSize', 14);
ylabel("Signal amplitude", 'FontSize', 14);
legend("Original signal y", "One-step ahead prediction $\hat{y}$",'Interpreter','latex', 'FontSize', 14);
...
nexttile;
plot(1:N, y, 1:N, y_hat, 'LineWidth', 2);
title(["Original signal (non-zero mean) and the prediction with bias input"], 'FontSize', 14);
xlabel("Sample Index", 'FontSize', 14);
ylabel("Signal amplitude", 'FontSize', 14);
legend("Original signal y", "Prediction $\hat{y}$",'Interpreter','latex', 'FontSize', 14);
xlim([1,200]);

[y_hat, ~, ~] = LMS_tanh([], y_0_mean, mu, AR_order, 0, 78, [], []);
nexttile;
plot(1:N, y_0_mean, 1:N, y_hat, 'LineWidth', 2);
title(["Zero-mean version of the signal y and the prediction without bias input"], 'FontSize', 14);
xlabel("Sample Index", 'FontSize', 14);
ylabel("Signal amplitude", 'FontSize', 14);
legend("Zero-mean y", "Prediction $\hat{y}$",'Interpreter','latex', 'FontSize', 14);
xlim([1,200]);