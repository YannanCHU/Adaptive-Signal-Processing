...
nexttile;
plot(f,PxxTrue, f,PxxEst(:,5), f,PxxEst(:,11), 'LineWidth', 2);
title("The true PSD and Estimated PSD against frequency", 'FontSize',14);
xlabel("Frequency (Hz)", 'FontSize',14); ylabel("PSD", 'FontSize',14);
legend("True PSD", "Optimal order = " + p(5), "Order = " + p(11) +" (visually optimal)", 'FontSize', 12)

nexttile;
plot(p, mean(PxxDiff.^2), 'LineWidth',2);
